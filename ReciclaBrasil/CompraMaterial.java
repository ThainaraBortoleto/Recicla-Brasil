...
public class CompraMaterial extends JFrame {
	private JLabel lblEmpresa, lblNome, lblValorAPagar, lblTotalValor;

	// variáveis Editar Cadastro
	private JLabel lblEditar;
	private JButton cancelarButton, depositarButton;

	private JLabel lblIncio, lblPainelDeControle, lblCopyRight, lblDesenvolvedores, lblPeso, lblPeso2, lblPeso3,
			lblPeso4, lblTipoMaterial, lblTipoMaterial2, lblTipoMaterial3, lblTipoMaterial4;
	...
	private Material aluminio, papelao, plastico, vidro;
	private EmpresaDAO eDAO;
	private Empresa e1;

	public CompraMaterial() throws SQLException {
		super("ReciclaBrasil");
		...
		try (Connection con = new ConexaoBD().conectar()) {
			...
			lblEmpresa.setFont(new Font("Lucida Grande", Font.BOLD, 18));
			lblEmpresa.setBounds(200, 20, 200, 300);
			panelBody.add(lblEmpresa);
			...
			lblValorAPagar.setForeground(Color.BLACK);
			lblValorAPagar.setBounds(690, 30, 600, 380);
			lblValorAPagar.setFont(new Font("Lucida Grande", Font.PLAIN, 12));
			panelBody.add(lblValorAPagar);
			...
			compraMaterialButton.setFont(new Font("Lucida Grande", Font.BOLD, 12));
			...
			lblEditar = new JLabel("Compra Materiais");
			...
			lblTipoMaterial.setBounds(200, 120, 200, 700);
			...
			comboBox.setBounds(350, 445, 200, 50);
			panelBody.add(comboBox);

			lblPeso = new JLabel("Quantidade(Kg):");
			lblPeso.setForeground(Color.BLACK);
			lblPeso.setBounds(650, 120, 200, 700);
			panelBody.add(lblPeso);

			pesoTextField = new JTextField();
			pesoTextField.setBounds(750, 455, 100, 35);
			...
			// comboBox2.setModel(new DefaultComboBoxModel(new String[] {"Alumínio",
			// "Papelão", "Plástico", "Vidro"}));
			...
			// comboBox3.setModel(new DefaultComboBoxModel(new String[] {"Alumínio",
			// "Papelão", "Plástico", "Vidro"}));
			...
			// comboBox4.setModel(new DefaultComboBoxModel(new String[] {"Alumínio",
			// "Papelão", "Plástico", "Vidro"}));
		...
		public void actionPerformed(ActionEvent e) {
			try (Connection con = new ConexaoBD().conectar()) {
				if (e.getSource() == pagInicial) {
					...
					String selectValue = String.valueOf(comboBox.getSelectedItem().toString());
					
					eDAO = new EmpresaDAO(con);
					MaterialDAO mDAO = new MaterialDAO(con);
					
					if (selectValue.equalsIgnoreCase("Alumínio")) {
						aluminio = new Material();
						aluminio.setIdMateriais(1);
						int quantidade = Integer.parseInt(pesoTextField.getText());
						
						mDAO.diminuiQuantidade(aluminio, quantidade);
						eDAO.setValorAPagar(e1, aluminio, quantidade);
						lblTotalValor.setText(String.format("R$ %.2f", eDAO.getValorAPagar(e1)));
						pesoTextField.setText("");
						
						JOptionPane.showMessageDialog(null, "Adicionado ao carrinho com sucesso");
					}
					if (selectValue.equalsIgnoreCase("Papelão")) {
						papelao = new Material();
						papelao.setIdMateriais(2);
						int quantidade = Integer.parseInt(pesoTextField.getText());
						
						mDAO.diminuiQuantidade(papelao, quantidade);
						eDAO.setValorAPagar(e1, papelao, quantidade);
						lblTotalValor.setText(String.format("R$ %.2f", eDAO.getValorAPagar(e1)));
						pesoTextField.setText("");
					...
					if (selectValue.equalsIgnoreCase("plástico")) {
						plastico = new Material();
						plastico.setIdMateriais(3);
						int quantidade = Integer.parseInt(pesoTextField.getText());
						
						mDAO.diminuiQuantidade(plastico, quantidade);
						eDAO.setValorAPagar(e1, plastico, quantidade);
						lblTotalValor.setText(String.format("R$ %.2f", eDAO.getValorAPagar(e1)));
						pesoTextField.setText("");
					...
					if (selectValue.equalsIgnoreCase("vidro")) {
						vidro = new Material();
						vidro.setIdMateriais(4);
						int quantidade = Integer.parseInt(pesoTextField.getText());
						
						mDAO.diminuiQuantidade(vidro, quantidade);
						eDAO.setValorAPagar(e1, vidro, quantidade);
						lblTotalValor.setText(String.format("R$ %.2f", eDAO.getValorAPagar(e1)));
						pesoTextField.setText("");
