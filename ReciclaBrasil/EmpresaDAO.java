...
public class EmpresaDAO {
	...
	public void inserir(Empresa empresa) throws SQLException {
		String slqInsert = "insert into empresa (id_empresa, nome, nome_responsavel, razao_social, cnpj, cep, endereco, telefone, email) values (?, ?, ?, ?, ?, ?, ?, ?, ?)";

		try (PreparedStatement stm = con.prepareStatement(slqInsert)) {
			stm.setInt(1, empresa.getIdEmpresa());
			stm.setString(2, empresa.getNome());
			stm.setString(3, empresa.getNomeResponsavel());
			stm.setString(4, empresa.getRazaoSocial());
			stm.setString(5, empresa.getCnpj());
			stm.setString(6, empresa.getCep());
			stm.setString(7, empresa.getEndereco());
			stm.setString(8, empresa.getTelefone());
			stm.setString(9, empresa.getEmail());
	...
	public void alterar(Empresa empresa) throws SQLException {
		String sqlUpdate = "update empresa set nome = ?, nome_responsavel = ?, razao_social = ?, cep = ?, endereco = ?, telefone = ?, email = ? where id_empresa = ?";
		...
		try (PreparedStatement stm = con.prepareStatement(sqlUpdate)) {
			...
			stm.setInt(8, empresa.getIdEmpresa());
	...
	public double getValorAPagar(Empresa empresa) throws SQLException {
		double valorAPagar = 0;
		String sqlSaldo = "select valor_pagamento from empresa where id_empresa = ?";
		try (PreparedStatement stm = con.prepareStatement(sqlSaldo)) {
			...
			try (ResultSet rs = stm.getResultSet()) {
				while (rs.next()) {
					valorAPagar = rs.getDouble("valor_pagamento");
				}
			}
			return valorAPagar;
		}
	...
	public void setValorAPagar(Empresa empresa, Material material, double peso) throws SQLException {
		String sqlUpdateSaldo = "update empresa set valor_pagamento = ? + (select ? * preco from material where id_material = ?) where id_empresa = ?";

		try (PreparedStatement stm = con.prepareStatement(sqlUpdateSaldo)) {
			stm.setDouble(1, getValorAPagar(empresa));
			stm.setDouble(2, peso);
			stm.setInt(3, material.getIdMateriais());
			stm.setInt(4, empresa.getIdEmpresa());
	...
	public void pagar(Empresa empresa, double valor) throws SQLException {
		String sqlUpdateSaldo = "update empresa set valor_pagamento = ? - ? where id_empresa = ?";
	...
	public String getNome(int id) throws SQLException {
		String nome = null;
		String sqlSelectNome = "select nome from empresa where id_empresa = ?";
