...
public class MaterialDAO {
	...
	public String getNome(int id) throws SQLException {
		String nomeMaterial = "";
		String sqlSelect = "select nome from material where id_material = ?";
		try(PreparedStatement stm = con.prepareStatement(sqlSelect)){
			stm.setInt(1, id);
			try(ResultSet rs = stm.executeQuery()){
				if(rs != null) {
					while(rs.next()) {
						nomeMaterial = rs.getString(1);
					}
				}
			}
		}
	...
	public void diminuiQuantidade(Material material, int quantidade) throws SQLException{
		String sql = "update material set quantidade = ? - ? where id_material = ?";
		try(PreparedStatement stm = con.prepareStatement(sql)){
			stm.setInt(1, carregarQuantidade(material.getIdMateriais()));
	...
	public void aumentaQuantidade(Material material, int quantidade) throws SQLException{
		String sql = "update material set quantidade = ? + ? where id_material = ?";
		try(PreparedStatement stm = con.prepareStatement(sql)){
			stm.setInt(1, carregarQuantidade(material.getIdMateriais()));
			stm.setInt(2, quantidade);
