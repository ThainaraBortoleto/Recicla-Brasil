			...
			try (Connection con = new ConexaoBD().conectar()) {
				if (e.getSource() == pagInicial) {
					...
					if (selectValue.equalsIgnoreCase("Alumínio")) {
						System.out.println("É aluminio");
						aluminio = new Material();
						aluminio.setIdMateriais(1);
						double peso = Double.parseDouble(pesoTextField.getText());
						System.out.println("Peguei o peso");
						
						cDao.setValorVenda(c1, aluminio, peso);
						cDao.setSaldo(c1, cDao.getSaldo(c1));
					...
					if (selectValue.equalsIgnoreCase("Papelão")) {
						...
						cDao.setValorVenda(c1, papelao, peso);
						cDao.setSaldo(c1, cDao.getSaldo(c1));
						lblTotalValor.setText(String.format("R$ %.2f", cDao.getSaldo(c1)));
						pesoTextField.setText("");
					...
					if (selectValue.equalsIgnoreCase("plástico")) {
						...
						cDao.setValorVenda(c1, plastico, peso);
						cDao.setSaldo(c1, cDao.getSaldo(c1));
						lblTotalValor.setText(String.format("R$ %.2f", cDao.getSaldo(c1)));
						pesoTextField.setText("");
					...
					if (selectValue.equalsIgnoreCase("vidro")) {
						...
						cDao.setValorVenda(c1, vidro, peso);
						cDao.setSaldo(c1, cDao.getSaldo(c1));
						lblTotalValor.setText(String.format("R$ %.2f", cDao.getSaldo(c1)));
						pesoTextField.setText("");
